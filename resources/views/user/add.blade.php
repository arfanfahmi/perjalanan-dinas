            ...
            <form action="/user/store" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('username') is-invalid @enderror"  
                        name="username" aria-describedby="" required value="{{ old('username') }}">
                    @error('username')<div class="invalid-feedback">{{ $message }}</div>@enderror  
                </div>
                <div class="mb-3">
                    <label for="nama_pegawai" class="form-label">Nama Pegawai</label>
                    <input type="text" class="form-control @error('nama_pegawai') is-invalid @enderror"  
                        name="nama_pegawai" aria-describedby="" required value="{{ old('nama_pegawai') }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('nrp') is-invalid @enderror"  
                        name="nrp" aria-describedby="" required value="{{ old('nrp') }}">
                    @error('nrp')<div class="invalid-feedback">{{ $message }}</div>@enderror 
                </div>
                <div class="mb-3">
                    <label for="email" class="form-label">email</label>
                    <input type="email" class="form-control @error('email') is-invalid @enderror" 
                        name="email" aria-describedby="" required value="{{ old('email') }}">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control js-example-basic-single high-select @error('role') is-invalid @enderror" 
                        ...
                        <option value="{{ $roles[$i] }}">{{ strtoupper($roles[$i]) }}</option>
                        @endforeach
                    </select>
